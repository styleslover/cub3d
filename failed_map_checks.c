...
/*   Updated: 2025/03/18 19:35:09 by mabrigo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_line_start(char **world, t_analisys *line, t_analisys *prev_line, int i)
{
	...
	while (world[i][line->start] != '1')
		line->start++;
	while (world[i - 1][prev_line->start] != '1')
		prev_line->start++;
	if (!world[i][line->start])
		return (0);
	printf("---------START LINE AGGIORNATO: %d\n", line->start);
	printf("---------PREV START LINE AGGIORNATO: %d\n", prev_line->start);
	if (line->start > prev_line->start)
	{
		if (world[i - 1][line->start] == '1'
			|| world[i - 1][line->start - 1] == '1'
			|| world[i - 1][line->start + 1] == '1')
				check_first_zeros(world, line, i);
	}
	else if (line->start < prev_line->start)
	{
		if (world[i][prev_line->start] == '1'
			|| world[i][prev_line->start - 1] == '1'
			|| world[i][prev_line->start + 1] == '1')
			return (1);
	}
	else
	...
	printf("esco correttamente da check line start\n");
	return (0);
}
